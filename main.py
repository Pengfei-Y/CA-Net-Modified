...
from Datasets.ISIC2018_change import ISIC2018_dataset_change
...
Test_Dataset = {'ISIC2018': ISIC2018_dataset_change}
...
def valid_isic(valid_loader, model, criterion, optimizer, args, epoch, minloss):
    ...
    model.eval()
    for step, (n, t, k) in tqdm(enumerate(valid_loader), total=len(valid_loader)):
        image = t.float().cuda()
        target = k.float().cuda()
        name = n
        output = model(image)
        ...
        if (epoch%10 == 0):
            save_path = os.path.join("./validate_visualize", "pred_"+str(epoch))# model output
            ...
            for i in range(batch_size):
                ...
                pred[pred < 0.5] = 0
                pred[pred == 1] = 255
                pred = pred.cpu().squeeze(0).detach().numpy()
                img_name = name[i].replace('npy', 'jpg')
                path = os.path.join(save_path, "pre_"+img_name)
                cv.imwrite(path, pred)
...
def main(args):
    ...
    validset = Test_Dataset[args.data](dataset_folder=args.root_path, folder=args.val_folder, train_type='validation',
                                       transform=Test_Transform[args.data])
    testset = Test_Dataset[args.data](dataset_folder=args.root_path, folder=args.val_folder, train_type='validation',
                                      transform=Test_Transform[args.data])
...
if __name__ == '__main__':
    ...
    # Path related arguments  /mnt/code/data/ISIC2018_Task1_npy_all
