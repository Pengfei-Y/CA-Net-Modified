...
class ISIC2018_dataset(Dataset):
    def __init__(self, dataset_folder='/ISIC2018_Task1_npy_all',
        ...
        self.folder_file = 'D:/YZU-capstone/CA-Net/Datasets'

        if self.train_type in ['train', 'validation', 'test']:
            # this is for cross validation
            #with open(r"D:\YZU-capstone\CA-Net\Datasets\folder0\folder0_test.list",
            #          'r') as f:
            temp = join(self.folder_file, folder, folder + '_' + self.train_type + '.list')
            with open(join(self.folder_file, folder, folder + '_' + self.train_type + '.list'),
                      'r') as f:
