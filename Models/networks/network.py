...
from Models.layers.CBAM_ASPP_layer import CBAM_ASPP
...
class Comprehensive_Atten_Unet(nn.Module):
    def __init__(self, args, in_ch=3, n_classes=2, feature_scale=4, is_deconv=True, is_batchnorm=True,
        ...
        #这里定义一个apss module 放入主体的网络中
        self.aspp_module = CBAM_ASPP(16,16)#这里两个参数是设定通道数
    ...
    def forward(self, inputs):
        ...
        # print('dsv_cat',dsv_cat.shape)#看一下这个变量的通道数是16
        dsv_cat = self.aspp_module(dsv_cat)#这是加的asppnet，如果不需要的话可以直接注释掉，这样他就不会参与计算了
...
if __name__ == '__main__':
    ...
    #这里是输入一个随机的变量给网络推理一下，检查网络是否能走通
    args = parser.parse_args()
    model = Comprehensive_Atten_Unet(args, args.num_input, args.num_classes).cuda()
    input =torch.randn(2, 3, 224, 300).cuda()
    output = model(input)
    print(output.shape)


    # 这里是输入一个随机的变量给网络推理一下，检查网络是否能走通
    aspp = CBAM_ASPP(3,12).cuda()#第一个3是输入通道是3，12是输出通道是多少
